...
const CommonCheckTable = (props) => {
  ...
  // UPDATED CODE: Define columns with valid accessors
  const columns = useMemo(() => columnData.map(col => {
    // Generate accessor if missing
    const accessor = col.accessor || col.Header?.toLowerCase().replace(/\s+/g, '_');
    
    // Throw an error if accessor is not valid
    if (!accessor || typeof accessor !== 'string') {
      throw new Error(`A valid accessor (or string accessor) is required for column: ${col.Header}`);
    }
  ...
  const handleClear = () => {
    if (setSearchDisplay) setSearchDisplay(false);
    if (setDisplaySearchData) setDisplaySearchData(false);
    if (searchboxOutside) {
      setSearchboxOutside('');
    } else {
      setSearchbox('');
  ...
  const toggleColumnVisibility = (columnKey) => {
    setColumn(columnKey);
    isColumnSelected = selectedColumns?.some((column) => column?.accessor === columnKey);
    if (isColumnSelected) {
  ...
  const downloadCsvOrExcel = async (extension, selectedIds) => {
    try {
      if (selectedIds && selectedIds?.length > 0) {
        const selectedRecordsWithSpecificFields = allData?.filter((rec) => selectedIds.includes(rec._id))?.map((rec) => {
        ...
        convertJsonToCsvOrExcel(selectedRecordsWithSpecificFields, csvColumns, title || 'data', extension);
      } else {
        const allRecordsWithSpecificFields = allData?.map((rec) => {
          const selectedFieldsData = {};
          csvColumns?.forEach((property) => {
            selectedFieldsData[property?.accessor] = rec[property?.accessor];
          });
        ...
        convertJsonToCsvOrExcel(allRecordsWithSpecificFields, csvColumns, title || 'data', extension);
      }
    } catch (e) {
      console.error(e);
    }
        ...
        <Grid templateColumns="repeat(12, 1fr)" gap={2}>
            ...
            <Flex alignItems={"center"} flexWrap={"wrap"}>
              ...
              {customSearch !== false && (
                <CustomSearchInput
                  ...
                  setDisplaySearchData={setSearchboxOutside ? props.setSearchDisplay : setDisplaySearchData}
                  searchbox={searchboxOutside ? searchboxOutside : searchbox}
                  allData={allData}
                  dataColumn={columns}
                  onSearch={handleSearch}
                  setGetTagValues={props.setGetTagValuesOutside ? props.setGetTagValuesOutside : setGetTagValues}
                  setGopageValue={setGopageValue}
                />
              )}
              {(AdvanceSearch !== false) && (
                <Button variant="outline" colorScheme='brand' leftIcon={<SearchIcon />} mt={{ sm: "5px", md: "0" }} size="sm" onClick={() => setAdvaceSearch(true)}>
                  Advance Search
                </Button>
              )}
              {(searchDisplay || displaySearchData) && (
                <Button variant="outline" colorScheme='red' size="sm" ms={2} onClick={() => handleClear()}>
                  Clear
                </Button>
              )}
              {(selectedValues?.length > 0 && access?.delete && !deleteMany) && (
                <DeleteIcon cursor={"pointer"} onClick={() => setDelete(true)} color={'red'} ms={2} />
              )}
            </Flex>
          </GridItem>
          ...
          <GridItem colSpan={{ base: 12, md: 4 }} display={"flex"} justifyContent={"end"} alignItems={"center"} textAlign={"right"}>
            {ManageGrid !== false && (
              <Menu isLazy>
                ...
                <MenuList minW={'fit-content'} transform={"translate(1670px, 60px)"} zIndex={2} >
                  <MenuItem onClick={() => setManageColumns(true)} width={"165px"}> Manage Columns </MenuItem>
                  {typeof setIsImport === "function" && (
                    <MenuItem width={"165px"} onClick={() => setIsImport(true)}> Import {title} </MenuItem>
                  )}
            ...
            {(access?.create || access === true) && (
              <Button onClick={() => handleClick()} size="sm" variant="brand" leftIcon={<AddIcon />}>
                Add New
              </Button>
            )}
